...
Shader "IL3DN/Terrain First-Pass"
{
	Properties
	{
		...
		_ColorTop1("Top", Color) = (0.1690798,1,0,0)
		_Color4("Sides", Color) = (1,0.5248495,0.1745283,0)
		_TriplanarMinSmooth("Triplanar Min Smooth", Float) = 0
		_TriplanarMaxSmooth("Triplanar Max Smooth", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _Falloff;
			float _TriplanarMinSmooth;
			float _TriplanarMaxSmooth;
			...
			uniform sampler2D _MidTexture1;
			uniform sampler2D _BotTexture1;
			sampler2D _Control;
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 triplanar102_g60 = TriplanarSamplingCF( _TopTexture1, _MidTexture1, _BotTexture1, WorldPosition, WorldNormal, _Falloff, _TriplanarTiling, float3( 1,1,1 ), float3(0,0,0) );
				float smoothstepResult119_g60 = smoothstep( _TriplanarMinSmooth , _TriplanarMaxSmooth , saturate( WorldNormal.y ));
				float4 lerpResult122_g60 = lerp( _Color4 , _ColorTop1 , smoothstepResult119_g60);
				float2 uv_Control = IN.ase_texcoord7.xy * _Control_ST.xy + _Control_ST.zw;
				float4 tex2DNode5_g60 = tex2D( _Control, uv_Control );
				float dotResult20_g60 = dot( tex2DNode5_g60 , float4(1,1,1,1) );
				float SplatWeight22_g60 = dotResult20_g60;
				float localSplatClip74_g60 = ( SplatWeight22_g60 );
				float SplatWeight74_g60 = SplatWeight22_g60;
				#if !defined(SHADER_API_MOBILE) && defined(TERRAIN_SPLAT_ADDPASS)
				clip(SplatWeight74_g60 == 0.0f ? -1 : 1);
				#endif
				float4 SplatControl26_g60 = ( tex2DNode5_g60 / ( localSplatClip74_g60 + 0.001 ) );
				#ifdef _SNOW_ON
				float staticSwitch88_g60 = ( WorldNormal.y * SnowTerrainFloat );
				#else
				float staticSwitch88_g60 = 0.0;
				#endif
				...
				float4 weightedBlendVar9_g60 = ( ( triplanar102_g60 * lerpResult122_g60 ) + SplatControl26_g60 );
				float4 weightedBlend9_g60 = ( weightedBlendVar9_g60.x*saturate( ( staticSwitch88_g60 + ( _Color0 * tex2D( _Splat0, uv_Splat0 ) ) ) ) + weightedBlendVar9_g60.y*saturate( ( staticSwitch88_g60 + ( _Color1 * tex2D( _Splat1, uv_Splat1 ) ) ) ) + weightedBlendVar9_g60.z*saturate( ( staticSwitch88_g60 + ( _Color2 * tex2D( _Splat2, uv_Splat2 ) ) ) ) + weightedBlendVar9_g60.w*saturate( ( staticSwitch88_g60 + ( _Color3 * tex2D( _Splat3, uv_Splat3 ) ) ) ) );
				float4 MixDiffuse28_g60 = weightedBlend9_g60;
				
				float3 Albedo = MixDiffuse28_g60.xyz;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 triplanar102_g60 = TriplanarSamplingCF( _TopTexture1, _MidTexture1, _BotTexture1, WorldPosition, ase_worldNormal, _Falloff, _TriplanarTiling, float3( 1,1,1 ), float3(0,0,0) );
				float smoothstepResult119_g60 = smoothstep( _TriplanarMinSmooth , _TriplanarMaxSmooth , saturate( ase_worldNormal.y ));
				float4 lerpResult122_g60 = lerp( _Color4 , _ColorTop1 , smoothstepResult119_g60);
				...
				float4 SplatControl26_g60 = ( tex2DNode5_g60 / ( localSplatClip74_g60 + 0.001 ) );
				#ifdef _SNOW_ON
				float staticSwitch88_g60 = ( ase_worldNormal.y * SnowTerrainFloat );
				#else
				float staticSwitch88_g60 = 0.0;
				#endif
...
/*ASEBEGIN
Version=18100
247;189;1726;501;1717.443;440.4434;1.6;True;False
...
Node;AmplifyShaderEditor.FunctionNode;76;-480.4067,-193.8597;Inherit;False;IL3DN - Four Splats First Pass Terrain;0;;60;c3e2b73ed8eb8c64681162a2abca2a89;0;3;59;FLOAT4;0,0,0,0;False;60;FLOAT4;0,0,0,0;False;62;FLOAT;0;False;2;FLOAT4;0;FLOAT;19
...
ASEEND*/
//CHKSM=022EE9D934E46D948C1507EFAA9AC4D59418D70F